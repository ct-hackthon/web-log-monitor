...
//        regression.predict()
    ...
    @Override
    public Double simpleLinearRegression(List<SimpleLinearRegressionTraningSet> list,double actualValue) {
       if(list==null||list.isEmpty()){
           return 0d;
       }
        SimpleRegression regression = new SimpleRegression();
       list.forEach(x->regression.addData(x.getX(),x.getY()));
        return regression.predict(actualValue);
...
//        return variance.getResult();
    ...
    public static void main(String[] arg){
        double[] data = {2.0,1.3,2.4,2.5,35};
        MathServiceImpl ins = new MathServiceImpl();
       System.out.println( ins.mean(data));
    }
}
